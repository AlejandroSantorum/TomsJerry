...
def get_move(request):
    ...
    if shift > 0:
        ...
        resp = {'origin': move.origin, 'target': move.target, 'previous': True, 'next': move_idx < n_moves - 1, 'move_counter': request.session['move_counter'], 'all_moves': str(games.moves), 'this_move': str(move)}
    else:
        move = game.moves[move_idx]
        move_idx += shift
        request.session['move_counter'] = move_idx
        resp = {'origin': move.target, 'target': move.origin, 'previous': move_idx >= 0, 'next': True, 'move_counter': request.session['move_counter'], 'all_moves': str(games.moves), 'this_move': str(move)}
